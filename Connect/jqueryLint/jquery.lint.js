...
((() => {
    ...
    var // Define console if not defined
    ...
    var addCheck = (methodName, level, check) => {
    ...
    var // Only cover certain fns under the jQ namespace
    ...
    var map = _jQuery.map;
    var each = _jQuery.each;
    var extend = _jQuery.extend;
    var find = _jQuery.find;
    var rootjQuery = _jQuery.rootjQuery;
    var undefined;
    var arrSlice = Array.prototype.slice;
    var slice = (a, s, e) => a.length ? arrSlice.call(a, s || 0, e || a.length) : [];
    ...
    var selectorCache = {};
    var jQueryMethods = extend({}, _jQuery.fn);
    var internal = false;
    var fromInit = false;
    ...
    function registerMethod(name, methodAPI) {
        ...
        var methodName = name.replace(/^jQuery\./, '');

        obj[methodName] = (((meth, name) => extend(function() {
    ...
    function coverMethod(name, meth, args) {
                ...
                _console.group(locale.availableSigsInclude);
                    each(sigs, (i, sig) => {
                        ...
                        _console.log(
                            ...
                            (sigArgs ?
                                 sigArgs[0] ?
                                    map(sigArgs, (sig, i) => sig ? sig.optional ? '[' + sig.name + ']' : sig.multiple ? sig.name + ',[...]' : sig.name : []).join(', ') :
                                    sigArgs.name
                            : '') + ')'
                        );
                    });
        ...
        if (specialCheckResults.length) {
            each(specialCheckResults, (i, checkResult) => {
    ...
    _jQuery.fn.init = ((_init => function(selector, context, rootjQuery) {
        var locale = lint.langs[lint.lang];

        var ret = coverMethod.call(this, 'jQuery', function(){
            return runFunction(() => new _init(selector, context, rootjQuery), [], true, this);
    ...
    // Some special checks //
    /////////////////////////
    addCheck('jQuery', 2, (selector, context) => {
        var locale = lint.langs[lint.lang];
        ...
        if (!internal && typeof selector === 'string' && !/^[^<]*(<[\w\W]+>)[^>]*$/.test(selector)) {
            ...
            if (lint.enabledReports.invalidFilters) {
                ...
                selector.replace(/('|")(?:\\\1|[^\1])+?\1/g, '').replace(/:(\w+)/g, (m, filter) => {
    ...
    addCheck('jQuery', 2, function(...args) {
    ...
    each({'attr': {'id': 'elem.id', 'value': 'jQuery.val()'}}, (method, attributes) => {
    ...
    // May as well use css({...})
    each(['css','attr','bind','one'], (i, methodName) => {

        addCheck(methodName, 3, function(){
            var args = arguments;
            var hoc = this;
            var locale = lint.langs[lint.lang];
            var sliced = slice(hoc, 0, 10);
            var _console = lint.console;
                    ...
                    _console.groupCollapsed(locale.moreInfo);
                        ...
                        _console.log(
                            locale.combineCalls
                                ...
                                .replace(/%1/, '{\n' +
                                    map([args, hoc._lastMethodArgs], a => '  "' + a[0] + '": ' + typeToString(a[1])).join(',\n')
                                + '\n}')
                        );

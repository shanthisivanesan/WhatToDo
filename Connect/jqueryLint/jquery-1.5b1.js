...
(((window, undefined) => {
    ...
    var jQuery = ((() => {
        ...
        var // Check for non-word characters
        rnonword = /\W/;

        var // Check for digits
        rdigit = /\d/;
        ...
        var // Has the ready events already been bound?
        readyBound = false;
        ...
        jQuery.fn = jQuery.prototype = {
            ...
            ready(...args) {
                ...
                // Change ready & apply
                return ( jQuery.fn.ready = readyList.done ).apply( this , args );
            },
        ...
        jQuery.extend({
            ...
            _Deferred() {
                ...
                var // stored [ context , args ]
                fired;

                var // to avoid firing when already doing so
                firing;

                var // flag to know if the deferred has been cancelled
                cancelled;

                var // the deferred itself
                deferred  = {
                    ...
                    resolve(...args) {
                        deferred.fire( jQuery.isFunction( this.promise ) ? this.promise() : this , args );
                        return this;
                    },
            ...
            Deferred(func) {
                ...
                jQuery.extend( deferred , {
                    ...
                    promise(obj) {
                        obj = obj || {};
                        jQuery.each( "then done fail isResolved isRejected".split( " " ) , (_, method) => {
        ...
        if ( indexOf ) {
            jQuery.inArray = (elem, array) => indexOf.call( array, elem );
        }
    ...
    jQuery.extend({
        ...
        data(elem, name, data) {
            ...
            var id = isNode ? elem[ jQuery.expando ] : null;
        ...
        removeData(elem, name) {
            ...
            var isNode = elem.nodeType;
            var id = isNode ? elem[ jQuery.expando ] : elem;
            var cache = jQuery.cache;
            var thisCache = isNode ? cache[ id ] : id;
    ...
    jQuery.extend({
        ...
        attr(elem, name, value, pass) {
            ...
            var // Whether we are setting (or getting)
            set = value !== undefined;
    ...
    var rspace = / /g;
    var rescape = /[^\w\s.|`]/g;
    var fcleanup = nm => nm.replace(rescape, "\\$&");
    var focusCounts = { focusin: 0, focusout: 0 };
    ...
    jQuery.event = {
        ...
        add(elem, types, handler, data) {
            ...
            if ( !eventHandle ) {
                elemData.handle = eventHandle = function(...args) {
                    // Handle the second event of a trigger and when
                    // an event is called after a page has unloaded
                    return typeof jQuery !== "undefined" && !jQuery.event.triggered ?
                        jQuery.event.handle.apply( eventHandle.elem, args ) :
                        undefined;
                };
            }
        ...
        remove(elem, types, handler, pos) {
            ...
            var eventKey = elem.nodeType ? "events" : "__events__";
            var elemData = jQuery.data( elem );
            var events = elemData && elemData[ eventKey ];
        ...
        trigger(event, data, elem /*, bubbling */) {
            ...
            try {
                if ( !(elem && elem.nodeName && jQuery.noData[elem.nodeName.toLowerCase()]) ) {
                    if ( elem[ "on" + type ] && elem[ "on" + type ](...data) === false ) {
                        event.result = false;
            ...
            if ( !event.isPropagationStopped() && parent ) {
                ...
                var targetType = type.replace( rnamespaces, "" );
                var isClick = jQuery.nodeName( target, "a" ) && targetType === "click";
                var special = jQuery.event.special[ targetType ] || {};
    ...
    var // In case of event delegation, we only need to rename the event.type,
    ...
    ((() => {
        ...
        var posProcess = (selector, context) => {
            var match;
            var tmpSet = [];
            var later = "";
    ...
    var // checked="checked" or checked (html5)
    ...
    jQuery.fn.extend({
        ...
        after(...args) {
            if ( this[0] && this[0].parentNode ) {
                ...
                set.push(...jQuery(args[0]).toArray());
                return set;
            }
        },
        ...
        clone(events) {
            ...
            var ret = this.map(function() {
                ...
                if ( !jQuery.support.noCloneEvent && (this.nodeType === 1 || this.nodeType === 11) && !jQuery.isXMLDoc(this) ) {
                    ...
                    var destElements = clone.getElementsByTagName("*");
    ...
    function cloneCopyEvent(orig, ret) {
        ret.each(function (nodeIndex) {
            ...
            var curData = jQuery.data( this, oldData );
            var events = oldData && oldData.events;
    ...
    var currentStyle;
    var fcamelCase = (all, letter) => letter.toUpperCase();
    ...
    if ( document.defaultView && document.defaultView.getComputedStyle ) {
        getComputedStyle = (elem, newName, name) => {
            var ret;
            var defaultView;
            var computedStyle;
    ...
    jQuery.fn.extend({
        load(url, params, callback) {
            ...
            jQuery.ajax({
                ...
                complete(jXHR, status, responseText) {
    ...
    jQuery.extend({
        ...
        ajax(url, options, s) {
            ...
            var jQuery_etag = jQuery.etag;

            var // Callbacks contexts
            callbackContext = s.context || s;

            var globalEventContext = s.context ? jQuery( s.context ) : jQuery.event;
            ...
            function done( status , statusText , response , headers) {
                ...
                if ( status >= 200 && status < 300 || status === 304 ) {
                    ...
                    if ( s.ifModified ) {
                        ...
                        var etag = jXHR.getResponseHeader("Etag");
                    ...
                    // If we have data
                        ...
                        try {
                            ...
                            var // Previous dataType
                            prev;

                            var // Conversion function
                            conv;

                            var // Conversion functions (when text is used in-between)
                            conv1;

                            var conv2;

                            var // Local references to dataTypes & converters
                            dataTypes = s.dataTypes;

                            var converters = s.converters;

                            var // DataType to responseXXX field mapping
                            responses = {
								"xml": "XML",
								"text": "Text"
							};
    ...
    function ajax_selectOrExecute( structure , s ) {
        ...
        var checked = {};
        var flag;
        var noSelect = structure !== "transports";
    ...
    jQuery.each( [ "Prefilter" , "Transport" ] , (_, name) => {
        _ = name.toLowerCase() + "s";
        jQuery[ "ajax" + name ] = function(...args) {
            return ajax_addElement( _ , args );
        };
    } );
    ...
    function determineDataType( s , ct , text , xml ) {
        ...
        var regexp;
        var dataTypes = s.dataTypes;
        var transportDataType = dataTypes[0];
        var response;
    ...
    // 2) sneakily ensure transportDataType is always jsonp for jsonp requests
    }).ajaxPrefilter("json jsonp", (s, originalSettings) => {
        ...
        if ( s.dataTypes[ 0 ] === "jsonp" ||
            ...
            var url = s.url.replace(jsre, "$1" + jsonpCallback + "$2");
            var data = s.url === url && typeof(s.data) === "string" ? s.data.replace(jsre, "$1" + jsonpCallback + "$2") : s.data;
    ...
    jQuery.ajaxTransport( (s, determineDataType) => {
    ...
    jQuery.offset = {
        initialize() {
            ...
            var html = "<div style='position:absolute;top:0;left:0;margin:0;border:5px solid #000;padding:0;width:1px;height:1px;'><div></div></div><table style='position:absolute;top:0;left:0;margin:0;border:5px solid #000;padding:0;width:1px;height:1px;' cellpadding='0' cellspacing='0'><tr><td></td></tr></table>";
        ...
        setOffset(elem, options, i) {
            ...
            var calculatePosition = (position === "absolute" && jQuery.inArray('auto', [curCSSTop, curCSSLeft]) > -1);
            var props = {};
            var curPosition = {};
            var curTop;
            var curLeft;
